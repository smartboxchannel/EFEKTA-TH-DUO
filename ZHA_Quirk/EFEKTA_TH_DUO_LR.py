# EFEKTA_TH_DUO LR.py
# Quirk for EFEKTA_TH_DUO_LR (Temperature and humidity sensors. Simple Thermostat.)
...
class TempMeasurement(TemperatureMeasurement, CustomCluster):
    class AttributeDefs(TemperatureMeasurement.AttributeDefs):
        switching_temperature_sensor: Final = ZCLAttributeDef(id=0x0229, type=t.Bool, access="rw")
...
(
    QuirkBuilder(EFEKTA, "EFEKTA_TH_DUO_LR")
    ...
    .replaces(TempMeasurement, endpoint_id=2, cluster_type=ClusterType.Server)
    .replaces(TempMeasurement, endpoint_id=2, cluster_type=ClusterType.Client)
    ...
    .replaces(RHMeasurement, endpoint_id=2, cluster_type=ClusterType.Server)
    .replaces(RHMeasurement, endpoint_id=2, cluster_type=ClusterType.Client)
    .adds_endpoint(1)
    .adds_endpoint(2)
    .sensor(
    ...
    .switch(
        TempMeasurement.AttributeDefs.switching_temperature_sensor.name,
        TempMeasurement.cluster_id,
        endpoint_id=1,
        translation_key="switching_temperature_sensor",
        fallback_name="Switching temperature sensor",
        unique_id_suffix="switching_temperature_sensor",
    )
    .switch(
